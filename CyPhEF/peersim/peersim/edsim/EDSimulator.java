...
public class EDSimulator {
		...
		if (ev.node == null) {
			...
			if (ev.event instanceof NextCycleEvent) {
				...
				try {
					...
					e.printStackTrace();
					throw new IllegalArgumentException("Protocol " + Configuration.lookupPid(pid) + " does not implement EDProtocol; " + ev.event.getClass());
				}
	...
	public static void nextExperiment() {
		...
		if (heap.maxTime() < endtime)
			throw new IllegalParameterException(PAR_ENDTIME, "End time is too large: configured event queue only" + " supports " + heap.maxTime());
...
//		executeNext();
	...
	public static void add(long delay, Object event, Node node, int pid) {
		if (delay < 0)
			throw new IllegalArgumentException("Protocol " + node.getProtocol(pid) + " is trying to add event " + event + " with a negative delay: " + delay);
	...
	// Custom
	public static void addAtTime(long eventTime, Object event, Node node, int pid) {
		if (pid > Byte.MAX_VALUE)
			throw new IllegalArgumentException("This version does not support more than " + Byte.MAX_VALUE + " protocols");

		if (endtime > eventTime) { // check like this to deal with overflow 
			heap.add(eventTime, event, node, (byte) pid);
		}		
	}

	// Custom
	public static void analysisAfterSimulation() {
	...
	public static double getLastEventTime() {
		return ((double)lastEventTime)/1000.0;
	}

	// Custom
	public static double getNextEventTime() {
		return ((double)((Heap)heap).nextEventTime())/1000.0;		
	}
	
	// Custom
	public static Node getNextProcessingNode() {
		return ((Heap)heap).nextProcessingNode();
	}
	
	// Custom
	public static double getCurrentSimulationTime() {
		return ((double)CommonState.getIntTime())/1000.0;
	}
