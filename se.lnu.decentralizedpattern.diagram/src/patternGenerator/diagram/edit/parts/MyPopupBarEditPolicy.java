...
import patternGenerator.diagram.providers.PatternsIconProvider;
 ...
 */
public class MyPopupBarEditPolicy extends DiagramAssistantEditPolicy {
	...
	protected void populatePopupBars() {
		...
		for (Iterator iter = types.iterator(); iter.hasNext();) {
			...
			if (type instanceof IElementType) {
				Image img1 = (new PatternsIconProvider()).getIcon((IAdaptable) type, 0);
				//addPopupBarDescriptor((IElementType) type, image);
				addPopupBarDescriptor((IElementType) type, img1);
				if(img1 == null) System.out.println("è null!");
			}
		}
	}
    ...
    protected String getDiagramAssistantID() {
        return MyPopupBarEditPolicy.class.getName();
    }
}
